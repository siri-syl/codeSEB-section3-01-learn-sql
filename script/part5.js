...
*/
const PART5_2_5 = `SELECT content.title FROM content LEFT JOIN user
...
LEFT JOIN content_category ON category.id = content_category.categoryId
LEFT JOIN content ON content_category.contentId = content.id
LEFT JOIN user ON content.userId = user.id WHERE user.name = 'jiSungPark'`;
...
RIGHT JOIN user ON user.id = content.userId WHERE user.name = 'duRiCha'`;

/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-10. 각 user(컬럼명: name)가 작성한 글의 개수 (컬럼명: ContentCount)를 출력하기 위한 SQL을 작성해주세요.
*/
const PART5_2_10 = `SELECT user.name AS name, COUNT(content.userId) AS ContentCount FROM user
LEFT JOIN Content ON user.id = content.userId
GROUP BY user.name`;
